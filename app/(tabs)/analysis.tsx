...
import { colors, spacing, borderRadius, formatCurrency, categoryConfig } from "../../lib/theme";
...
export default function AnalysisScreen() {
  ...
  const { transactions, loading: txLoading, refreshTransactions } = useTransactions();
  const { categories, loading: catLoading } = useCategories();
  const { budgets } = useBudgets();
  const { subscriptions, getMonthlyTotal: getSubscriptionMonthly } = useSubscriptions();
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((t) => t.date >= startDate.getTime());
  ...
  const previousPeriodTransactions = useMemo(() => {
    ...
    switch (selectedPeriod) {
      ...
      case "quarter":
        const quarterStart = Math.floor(now.getMonth() / 3) * 3;
        endDate = new Date(now.getFullYear(), quarterStart, 1);
    ...
    return transactions.filter((t) => t.date >= startDate.getTime() && t.date < endDate.getTime());
  }, [transactions, selectedPeriod]);
  ...
  const stats = useMemo(() => {
    ...
    return { income, expenses, savings, savingsRate, incomeChange, expenseChange, prevIncome, prevExpenses };
  ...
  const categoryBreakdown = useMemo(() => {
    const expensesByCategory = filteredTransactions
      ...
      .reduce((acc, t) => {
        ...
        return acc;
      }, {} as Record<string, { id: string; name: string; amount: number; color: string; count: number; transactions: any[] }>);
    ...
    return Object.values(expensesByCategory)
      .map((data) => {
        ...
        const change = prevAmount > 0 ? ((data.amount - prevAmount) / prevAmount) * 100 : 0;
        const avgTransaction = data.count > 0 ? data.amount / data.count : 0;
        
        return {
          ...data,
          percentage: stats.expenses > 0 ? (data.amount / stats.expenses) * 100 : 0,
          change,
          avgTransaction,
          prevAmount,
        };
  ...
  }, [filteredTransactions, previousPeriodTransactions, categories, stats.expenses]);

  // Daily spending trend
  const dailyTrend = useMemo(() => {
    const days = selectedPeriod === "week" ? 7 : selectedPeriod === "month" ? 30 : selectedPeriod === "quarter" ? 90 : 12;
    const now = new Date();
    const trend: { label: string; fullDate: string; income: number; expense: number; net: number }[] = [];
  ...
  const spendingPatterns = useMemo(() => {
    ...
    const lowestSpendingDay = byDayOfWeek.indexOf(Math.min(...byDayOfWeek.filter(v => v > 0)));
  ...
  const insights = useMemo(() => {
    const result: { type: 'success' | 'warning' | 'info' | 'tip'; title: string; message: string; icon: keyof typeof Ionicons.glyphMap }[] = [];
    ...
    return result;
  }, [stats, categoryBreakdown, spendingPatterns, getSubscriptionMonthly]);

  const maxTrendValue = useMemo(() => {
    const max = Math.max(...dailyTrend.map((d) => Math.max(d.income, d.expense)));
    return max || 100;
  }, [dailyTrend]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.periodSelector}>
        {(["week", "month", "quarter", "year"] as Period[]).map((period) => (
          <TouchableOpacity
            ...
            onPress={() => { hapticFeedback(); setSelectedPeriod(period); }}
          >
            <Text
              style={[
            ...
            >
              {period === "quarter" ? "3M" : period.charAt(0).toUpperCase() + period.slice(1, 3)}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={styles.tabBar}>
        ...
        ] as { id: AnalyticsTab; label: string; icon: keyof typeof Ionicons.glyphMap }[]).map((tab) => (
          <TouchableOpacity
            key={tab.id}
            style={[styles.tabButton, activeTab === tab.id && styles.tabButtonActive]}
            onPress={() => { hapticFeedback(); setActiveTab(tab.id); }}
          >
            <Ionicons 
              name={tab.icon} 
              size={18} 
          ...
          <>
              ...
              <View style={styles.healthScoreBar}>
                <View 
                  style={[
                    ...
                    { 
                      width: `${healthScore}%`,
                      backgroundColor: healthScore >= 70 ? colors.income : healthScore >= 40 ? colors.warning : colors.expense,
                    }
                  ]} 
                />
              </View>
              <Text style={styles.healthScoreLabel}>
                {healthScore >= 70 ? "Excellent! Keep up the great work!" : 
            ...
            <View style={styles.overviewGrid}>
              <Card style={[styles.overviewCard, styles.incomeCard]}>
                <View style={[styles.overviewIconContainer, { backgroundColor: colors.incomeLight }]}>
                ...
                {stats.incomeChange !== 0 && (
                  <View style={styles.changeIndicator}>
                    ...
                    />
                    <Text style={[styles.changeText, { color: getChangeIndicator(stats.incomeChange).color }]}>
              ...
              <Card style={[styles.overviewCard, styles.expenseCard]}>
                <View style={[styles.overviewIconContainer, { backgroundColor: colors.expenseLight }]}>
                ...
                {stats.expenseChange !== 0 && (
                  <View style={styles.changeIndicator}>
                    <Ionicons 
                      name={getChangeIndicator(-stats.expenseChange).icon as any}
                      size={12} 
                      color={getChangeIndicator(-stats.expenseChange).color}
                    />
                    <Text style={[styles.changeText, { color: getChangeIndicator(-stats.expenseChange).color }]}>
            ...
            <Card style={styles.savingsCard}>
              <View style={styles.savingsRow}>
                ...
                <View style={[
                  ...
                  { backgroundColor: stats.savingsRate >= 20 ? colors.incomeLight : stats.savingsRate >= 0 ? colors.warningLight : colors.expenseLight }
                ]}>
                  <Text style={[
                    styles.savingsRateText,
                    { color: stats.savingsRate >= 20 ? colors.income : stats.savingsRate >= 0 ? colors.warning : colors.expense }
                  ]}>
                ...
                <View 
                  style={[
                    ...
                    { 
                      ...
                      backgroundColor: stats.savingsRate >= 20 ? colors.income : stats.savingsRate >= 0 ? colors.warning : colors.expense
                    }
                  ]} 
                />
              </View>
            ...
            <Card style={styles.quickStatsCard}>
              ...
              <View style={styles.quickStatsGrid}>
                ...
                <View style={styles.quickStatItem}>
                  <Text style={styles.quickStatValue}>{formatCurrency(spendingPatterns.dailyAvg)}</Text>
                  <Text style={styles.quickStatLabel}>Daily Avg</Text>
                </View>
                <View style={styles.quickStatItem}>
                  <Text style={styles.quickStatValue}>{categoryBreakdown.length}</Text>
                    ...
                    <View key={cat.id} style={styles.breakdownItem}>
                      <View style={styles.breakdownLeft}>
                        ...
                        <View>
                          ...
                          <Text style={styles.breakdownCount}>
                            {cat.count} transactions • Avg: {formatCurrency(cat.avgTransaction)}
                          </Text>
                        </View>
                      </View>
                      <View style={styles.breakdownRight}>
                        ...
                        <View style={styles.breakdownMeta}>
                          ...
                          {cat.change !== 0 && (
                            <View style={[styles.changeChip, { backgroundColor: cat.change > 0 ? colors.expenseLight : colors.incomeLight }]}>
                              <Ionicons 
                                name={cat.change > 0 ? "arrow-up" : "arrow-down"} 
                                size={10} 
                                color={cat.change > 0 ? colors.expense : colors.income} 
                              />
                              <Text style={[styles.changeChipText, { color: cat.change > 0 ? colors.expense : colors.income }]}>
            ...
            <Card style={styles.dayAnalysisCard}>
              ...
              <View style={styles.dayChart}>
                {spendingPatterns.dayNames.map((day, index) => {
                  ...
                  const height = maxDay > 0 ? (spendingPatterns.byDayOfWeek[index] / maxDay) * 100 : 0;
                  const isHighest = spendingPatterns.byDayOfWeek[index] === maxDay && maxDay > 0;
                  ...
                  return (
                    <View key={day} style={styles.dayBarContainer}>
                      ...
                      <Text style={[styles.dayLabel, isHighest && styles.dayLabelHighest]}>{day}</Text>
                    </View>
                  );
                })}
              </View>
            ...
            <Card style={styles.topCategoriesCard}>
              ...
              {spendingPatterns.topCategories.map((cat, index) => (
                <View key={cat.id} style={styles.topCategoryItem}>
                  <View style={[styles.topCategoryRank, { backgroundColor: index === 0 ? '#FFD700' : index === 1 ? '#C0C0C0' : '#CD7F32' }]}>
                  ...
                  <View style={styles.topCategoryInfo}>
                    ...
                    <View style={styles.topCategoryBar}>
                      <View style={[styles.topCategoryBarFill, { width: `${cat.percentage}%`, backgroundColor: cat.color }]} />
                    </View>
                  </View>
          ...
          <>
            ...
            <Card style={styles.chartCard}>
              ...
              <View style={styles.chart}>
                {dailyTrend.slice(selectedPeriod === "year" ? 0 : -7).map((day, index) => (
            ...
            <Card style={styles.netTrendCard}>
              ...
              <Text style={styles.netTrendSubtitle}>Running balance of income minus expenses</Text>
              <View style={styles.netTrendChart}>
                {(() => {
                  let cumulative = 0;
                  const data = dailyTrend.slice(selectedPeriod === "year" ? 0 : -7).map((d) => {
                    cumulative += d.net;
                    return { ...d, cumulative };
                  });
                  ...
                  return data.map((day, index) => {
                    ...
                    return (
                      <View key={index} style={styles.netTrendBarContainer}>
                        <View style={[styles.netTrendBarTop, { height: isPositive ? height : 0 }]}>
                          {isPositive && <View style={[styles.netTrendBar, { backgroundColor: colors.income }]} />}
                        </View>
                        <View style={styles.netTrendLine} />
                        <View style={[styles.netTrendBarBottom, { height: !isPositive ? height : 0 }]}>
                          {!isPositive && <View style={[styles.netTrendBar, { backgroundColor: colors.expense }]} />}
                        </View>
            ...
            <Card style={styles.comparisonCard}>
              ...
              <View style={styles.comparisonRow}>
                <View style={styles.comparisonItem}>
                  ...
                  <Text style={styles.comparisonValue}>{formatCurrency(stats.expenses)}</Text>
                </View>
                ...
                <View style={styles.comparisonItem}>
                  ...
                  <Text style={styles.comparisonValue}>{formatCurrency(stats.prevExpenses)}</Text>
                </View>
              </View>
              <View style={[
                styles.comparisonResult,
                { backgroundColor: stats.expenseChange <= 0 ? colors.incomeLight : colors.expenseLight }
              ]}>
                <Ionicons 
                  name={stats.expenseChange <= 0 ? "trending-down" : "trending-up"} 
                  size={20} 
                  color={stats.expenseChange <= 0 ? colors.income : colors.expense} 
                />
                <Text style={[
                  ...
                  {formatCurrency(Math.abs(stats.expenses - stats.prevExpenses))}
                  {stats.expenseChange <= 0 ? " less" : " more"} than last period
                </Text>
              </View>
            </Card>
          </>
                ...
                <Card 
                  ...
                  style={[
                    ...
                    { borderLeftColor: 
                      ...
                      insight.type === 'tip' ? colors.accent : colors.primary 
                    }
                  ]}
                >
                  <View style={[
                    ...
                    { backgroundColor: 
                      insight.type === 'success' ? colors.incomeLight : 
                      insight.type === 'warning' ? colors.warningLight : 
                      insight.type === 'tip' ? colors.accentLight : colors.primaryLight 
                    }
                  ]}>
                    <Ionicons 
                      name={insight.icon} 
