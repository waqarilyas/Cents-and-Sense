...
describe('E2E: Categories & Settings', () => {
  ...
  it('updates a category', async () => {
    ...
    const cat = testDB.getTable('categories').find((c: any) => c.id === 'cat_food');
  ...
  it('prevents deleting a category linked to a subscription', async () => {
    ...
    testDB.insertDirect('categories', {
      id: 'cat_ent', name: 'Entertainment', type: 'expense', color: '#9C27B0', createdAt: Date.now(),
    });

    testDB.insertDirect('subscriptions', {
      id: 'sub_1', name: 'Netflix', amount: 15.99, currency: 'USD',
      categoryId: 'cat_ent', frequency: 'monthly',
  ...
  it('deletes an unlinked category successfully', async () => {
    ...
    testDB.insertDirect('categories', {
      id: 'cat_1', name: 'Temp Category', type: 'expense', color: '#ccc', createdAt: Date.now(),
    });
    testDB.insertDirect('categories', {
      id: 'cat_2', name: 'Keep Me', type: 'income', color: '#aaa', createdAt: Date.now(),
    });
  ...
  it('separates income and expense categories', async () => {
    ...
    testDB.insertDirect('categories', {
      id: 'cat_1', name: 'Salary', type: 'income', color: '#4CAF50', createdAt: Date.now(),
    });
    testDB.insertDirect('categories', {
      id: 'cat_2', name: 'Freelance', type: 'income', color: '#8BC34A', createdAt: Date.now(),
    });
    testDB.insertDirect('categories', {
      id: 'cat_3', name: 'Food', type: 'expense', color: '#FF5722', createdAt: Date.now(),
    });
...
describe('E2E: Settings Persistence', () => {
  ...
  it('restores saved settings on reload', async () => {
    ...
    (AsyncStorage.getItem as jest.Mock).mockImplementation((key: string) => {
      if (key === '@budget_tracker_settings') return Promise.resolve(savedSettings);
      if (key === '@budget_app_onboarding_complete') return Promise.resolve('true');
...
describe('E2E: Account Management', () => {
  ...
  it('updates an account name and type', async () => {
    ...
    testDB.insertDirect('accounts', {
      id: 'acc_1', name: 'Old Name', type: 'checking',
      balance: 1000, currency: 'USD', isDefault: 1, createdAt: Date.now(),
    });
  ...
  it('switches default account', async () => {
    ...
    testDB.insertDirect('accounts', {
      id: 'acc_1', name: 'Primary', type: 'checking',
      balance: 1000, currency: 'USD', isDefault: 1, createdAt: Date.now(),
    });
    testDB.insertDirect('accounts', {
      id: 'acc_2', name: 'Secondary', type: 'savings',
  ...
  it('deleting an account also removes its transactions', async () => {
    ...
    testDB.insertDirect('accounts', {
      id: 'acc_2', name: 'To Delete', type: 'savings',
      balance: 500, currency: 'USD', isDefault: 0, createdAt: Date.now(),
    });

    // Transaction linked to acc_2
    testDB.insertDirect('transactions', {
      id: 'tx_1', accountId: 'acc_2', categoryId: 'cat_food',
      amount: 50, currency: 'USD', description: 'Test',
