...
describe('E2E: Full User Journey', () => {
  ...
  beforeEach(async () => {
    ...
    (AsyncStorage.getItem as jest.Mock).mockImplementation((key: string) => {
      if (key === '@budget_app_onboarding_complete') return Promise.resolve(null);
  ...
  it('complete journey: onboard → transact → budget → goal', async () => {
    ...
    await act(async () => {
      await transaction.addTransaction(
        accountId, freelanceCat.id, 2000,
        'Project payment', Date.now(), 'income'
      );
    });
    ...
    await act(async () => {
      await transaction.addTransaction(
        accountId, diningCat.id, 85,
        'Restaurant', Date.now(), 'expense'
      );
    });
    ...
    // ---- STEP 5: SET GOAL ----
    await act(async () => {
      await goal.addGoal('Emergency Fund', 5000, Date.now() + 180 * 86400000, 'USD');
    });
...
describe('E2E: Cross-Context Data Integrity', () => {
  ...
  it('transaction balance updates are reflected in account data', async () => {
    ...
    // Add income
    await act(async () => {
      await txState.addTransaction('acc_1', 'cat_salary', 1000, 'Pay', Date.now(), 'income');
    });
    ...
    // Add expense
    await act(async () => {
      await txState.addTransaction('acc_1', 'cat_food', 250, 'Groceries', Date.now(), 'expense');
    });

    expect(testDB.getTable('accounts')[0].balance).toBe(initialBalance + 1000 - 250);
  ...
  it('category with linked budget cannot be deleted', async () => {
    ...
    expect(testDB.getTable('categories').find((c: any) => c.id === 'cat_food')).toBeTruthy();
  });
  ...
  it('subscription auto-processing creates transaction and updates balance', async () => {
    ...
    testDB.insertDirect('subscriptions', {
      ...
      startDate: pastDue - 30 * 86400000, nextDueDate: pastDue,
    ...
    expect(testDB.getTable('subscriptions')[0].nextDueDate).toBeGreaterThan(pastDue);
  });
    ...
    for (const op of ops) {
      await act(async () => {
        await txState.addTransaction(
          'acc_1', op.cat, op.amount, op.desc, Date.now(), op.type as any
        );
      });
    }
