...
class InMemoryDB {
  ...
  private parseWhere(sql: string, params: any[]): (row: Row) => boolean {
    ...
    for (const cond of conditions) {
      ...
      if (m) {
        ...
        filters.push((row) => {
          switch (op) {
            case '=': return row[col] === val || row[col] == val;
  ...
  async getAllAsync(sql: string, params: any[] = []): Promise<Row[]> {
    ...
    if (orderMatch) {
      ...
      results.sort((a, b) => dir === 'DESC' ? (b[col] - a[col]) : (a[col] - b[col]));
    }
  ...
  async runAsync(sql: string, params: any[] = []): Promise<{ changes: number; lastInsertRowId: number }> {
    ...
    if (sqlUpper.startsWith('UPDATE')) {
      ...
      const setClause = (sql.match(/SET\s+(.+?)\s+WHERE/i) || sql.match(/SET\s+(.+?)$/i))![1];
      ...
      for (const a of assignments) {
        ...
        if (incrMatch) {
          ...
          setters.push({ setter: (row, val) => { row[col] = (row[col] || 0) + val; }, paramIdx: idx });
          continue;
        }
        // col = col - ? (decrement)
        ...
        if (simpleMatch) {
          ...
          const idx = setParamIdx++;
          setters.push({ setter: (row, val) => { row[col] = val; }, paramIdx: idx });
          continue;
        }
        ...
        if (literalNumMatch) {
          ...
          setters.push({ setter: (row) => { row[col] = val; }, paramIdx: -1 });
          continue;
        }
        // col = 'literal_string'
        ...
        if (literalNullMatch) {
          ...
          setters.push({ setter: (row) => { row[col] = null; }, paramIdx: -1 });
          continue;
        }
        // col = COALESCE(col, 0) + ? or similar — treat as increment
        const coalesceMatch = a.match(/^(\w+)\s*=\s*COALESCE\s*\(\s*\1\s*,\s*\d+\s*\)\s*\+\s*\?$/i);
        if (coalesceMatch) {
    ...
    if (sqlUpper.startsWith('DELETE')) {
      ...
      return { changes: before - this.tables[table].length, lastInsertRowId: 0 };
    }
...
export function createAppWrapper() {
  ...
  const { TransactionProvider } = require('../../lib/contexts/TransactionContext');
  const { BudgetProvider } = require('../../lib/contexts/BudgetContext');
  const { GoalProvider } = require('../../lib/contexts/GoalContext');
  const { SubscriptionProvider } = require('../../lib/contexts/SubscriptionContext');
      ...
      React.createElement(UserProvider, null,
        React.createElement(CurrencyProvider, null,
          React.createElement(AccountProvider, null,
            React.createElement(CategoryProvider, null,
              React.createElement(TransactionProvider, null,
                React.createElement(BudgetProvider, null,
                  React.createElement(GoalProvider, null,
