...
import { Account, Transaction, Budget, Goal, Subscription } from '../../lib/database';
...
describe('getMonthlyStatsByCurrency', () => {
  ...
  it('calculates income, expense, and balance for current month', () => {
    const txns = [
      makeTransaction({ amount: 3000, type: 'income', currency: 'USD', date: new Date(year, month, 5).getTime() }),
      makeTransaction({ amount: 100, type: 'expense', currency: 'USD', date: new Date(year, month, 10).getTime() }),
      makeTransaction({ amount: 200, type: 'expense', currency: 'USD', date: new Date(year, month, 15).getTime() }),
    ];
  ...
  it('separates currencies in stats', () => {
    const txns = [
      makeTransaction({ amount: 100, type: 'expense', currency: 'USD', date: new Date(year, month, 5).getTime() }),
      makeTransaction({ amount: 50, type: 'expense', currency: 'EUR', date: new Date(year, month, 5).getTime() }),
    ];
    const stats = getMonthlyStatsByCurrency(txns, month, year);
  ...
  it('excludes transactions from other months', () => {
    ...
    const txns = [
      makeTransaction({ amount: 100, type: 'expense', currency: 'USD', date: new Date(year, otherMonth, 5).getTime() }),
    ];
    const stats = getMonthlyStatsByCurrency(txns, month, year);
...
describe('getSubscriptionTotalByCurrency', () => {
  it('sums monthly subscriptions as-is', () => {
    const subs = [
      ...
      makeSubscription({ amount: 20, frequency: 'monthly', currency: 'USD', id: 'sub-2' }),
    ];
  ...
  it('excludes inactive subscriptions', () => {
    const subs = [
      makeSubscription({ amount: 10, isActive: false }),
    ];
