...
describe("validateAmount", () => {
  ...
  it("accepts negative when allowNegative is true", () => {
    expect(
      validateAmount(-10, "balance", { allowNegative: true, min: -999999999 }),
    ).toBe(-10);
  });

  // Range enforcement
  it("rejects values below custom min", () => {
    expect(() => validateAmount(5, "amount", { min: 10 })).toThrow(
      "must be at least 10",
    );
  });

  it("rejects values above custom max", () => {
    expect(() => validateAmount(200, "amount", { max: 100 })).toThrow(
      "cannot exceed 100",
    );
  });
...
describe("validateString", () => {
  ...
  it("rejects non-string types (number)", () => {
    expect(() => validateString(123 as any, "name")).toThrow(
      "must be a string",
    );
  });

  it("rejects null", () => {
    expect(() => validateString(null as any, "name")).toThrow(
      "must be a string",
    );
  });
  ...
  it("rejects strings shorter than minLength", () => {
    expect(() => validateString("ab", "name", { minLength: 3 })).toThrow(
      "at least 3 characters",
    );
  });

  it("rejects strings longer than maxLength", () => {
    expect(() => validateString("abcdef", "name", { maxLength: 5 })).toThrow(
  ...
  it("rejects strings not matching pattern", () => {
    expect(() =>
      validateString("hello!", "name", { pattern: /^[a-z]+$/ }),
    ).toThrow("invalid format");
  });

  it("accepts strings matching pattern", () => {
    expect(validateString("hello", "name", { pattern: /^[a-z]+$/ })).toBe(
      "hello",
    );
  });
});
...
describe("validateId", () => {
  ...
  it("rejects IDs with spaces", () => {
    expect(() => validateId("id with space", "id")).toThrow(
      "invalid characters",
    );
  });
  ...
  it("rejects IDs over 100 characters", () => {
    expect(() => validateId("a".repeat(101), "id")).toThrow(
