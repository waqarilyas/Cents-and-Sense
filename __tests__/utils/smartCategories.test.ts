...
describe('matchCategoryByDescription', () => {
  ...
  it('matches "Netflix subscription" to a valid expense category', () => {
    const result = matchCategoryByDescription('Netflix subscription', 'expense');
    expect(result).not.toBeNull();
  ...
  it('matches "monthly rent payment" to a valid expense category', () => {
    const result = matchCategoryByDescription('monthly rent payment', 'expense');
    expect(result).not.toBeNull();
    expect(result!.type).toBe('expense');
  });
...
describe('getCategorySuggestions', () => {
  it('returns up to the specified limit', () => {
    const suggestions = getCategorySuggestions('food lunch dinner', 'expense', 5);
    expect(suggestions.length).toBeLessThanOrEqual(5);
  });

  it('returns results sorted by relevance (first result = best match)', () => {
    const suggestions = getCategorySuggestions('restaurant dinner', 'expense', 3);
  ...
  it('defaults to limit of 3', () => {
    const suggestions = getCategorySuggestions('food grocery shopping', 'expense');
