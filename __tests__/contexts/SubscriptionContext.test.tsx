...
import { SubscriptionProvider, useSubscriptions } from '../../lib/contexts/SubscriptionContext';
...
const mockDb = {
  ...
  getFirstAsync: jest.fn(async (query: string) => {
    if (query.includes('categories')) return { id: 'cat-1', name: 'Entertainment' };
    if (query.includes('accounts')) return { id: 'acc-1', currency: 'USD', type: 'checking', balance: 5000 };
    if (query.includes('transactions') && query.includes('subscriptionId')) return null; // no dups
    return null;
  }),
...
jest.mock('../../lib/utils/validation', () => ({
  validateString: jest.fn((val: any, field: string, opts?: any) => {
    ...
    if (opts?.minLength && trimmed.length < opts.minLength) throw new Error(`${field} too short`);
    if (opts?.maxLength && trimmed.length > opts.maxLength) throw new Error(`${field} too long`);
    return trimmed;
  }),
...
describe('SubscriptionContext — Initial State', () => {
  ...
  it('activeSubscriptions derived from isActive filter', async () => {
    mockSubscriptions.push(
      { id: 's1', name: 'Netflix', amount: 15, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000, isActive: 1, reminderDays: 3, createdAt: 1 },
      { id: 's2', name: 'Old Sub', amount: 10, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000, isActive: 0, reminderDays: 3, createdAt: 2 },
    );
...
describe('SubscriptionContext — addSubscription', () => {
  it('creates a subscription with valid inputs', async () => {
    ...
    await act(async () => {
      newId = await result.current.addSubscription(
        'Netflix', 15.99, 'cat-1', 'monthly', Date.now(), 'USD'
      );
    });
    ...
    await expect(
      act(async () => {
        await result.current.addSubscription(
          'Netflix', 15.99, 'nonexistent', 'monthly', Date.now(), 'USD'
        );
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects empty name', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addSubscription('', 15, 'cat-1', 'monthly', Date.now(), 'USD');
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects zero amount', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addSubscription('Netflix', 0, 'cat-1', 'monthly', Date.now(), 'USD');
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects invalid frequency', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addSubscription(
          'Netflix', 15, 'cat-1', 'biweekly' as any, Date.now(), 'USD'
        );
      })
    ).rejects.toThrow();
  });
...
describe('SubscriptionContext — updateSubscription', () => {
  beforeEach(() => {
    mockSubscriptions.push({
      id: 'sub-1', name: 'Netflix', amount: 15, currency: 'USD', categoryId: 'cat-1',
      frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000 * 25,
      isActive: 1, reminderDays: 3, createdAt: Date.now(),
    });
  });
  ...
  it('updates subscription name', async () => {
    ...
    await act(async () => {
      await result.current.updateSubscription('sub-1', { name: 'Netflix Premium' });
    });
...
describe('SubscriptionContext — getMonthlyTotal', () => {
  it('calculates monthly cost from monthly subscriptions', async () => {
    mockSubscriptions.push(
      { id: 's1', name: 'Netflix', amount: 15, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000, isActive: 1, reminderDays: 3, createdAt: 1 },
      { id: 's2', name: 'Spotify', amount: 10, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000, isActive: 1, reminderDays: 3, createdAt: 2 },
    );
  ...
  it('normalizes yearly subscription to monthly', async () => {
    mockSubscriptions.push({
      id: 's1', name: 'Annual Plan', amount: 120, currency: 'USD', categoryId: 'cat-1',
      frequency: 'yearly', startDate: Date.now(), nextDueDate: Date.now() + 86400000,
      isActive: 1, reminderDays: 3, createdAt: 1,
    });
  ...
  it('normalizes weekly subscription to monthly', async () => {
    mockSubscriptions.push({
      id: 's1', name: 'Weekly', amount: 10, currency: 'USD', categoryId: 'cat-1',
      frequency: 'weekly', startDate: Date.now(), nextDueDate: Date.now() + 86400000,
      isActive: 1, reminderDays: 3, createdAt: 1,
    });
  ...
  it('excludes inactive subscriptions from total', async () => {
    mockSubscriptions.push({
      id: 's1', name: 'Inactive', amount: 100, currency: 'USD', categoryId: 'cat-1',
      frequency: 'monthly', startDate: Date.now(), nextDueDate: Date.now() + 86400000,
      isActive: 0, reminderDays: 3, createdAt: 1,
    });
...
describe('SubscriptionContext — getUpcomingSubscriptions', () => {
  it('returns subscriptions due within specified days', async () => {
    ...
    mockSubscriptions.push(
      { id: 's1', name: 'Due Tomorrow', amount: 10, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: tomorrow, isActive: 1, reminderDays: 3, createdAt: 1 },
      { id: 's2', name: 'Due Next Week', amount: 10, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: nextWeek, isActive: 1, reminderDays: 3, createdAt: 2 },
      { id: 's3', name: 'Due Next Month', amount: 10, currency: 'USD', categoryId: 'cat-1', frequency: 'monthly', startDate: Date.now(), nextDueDate: nextMonth, isActive: 1, reminderDays: 3, createdAt: 3 },
    );
