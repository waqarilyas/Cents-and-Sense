...
import { TransactionProvider, useTransactions } from '../../lib/contexts/TransactionContext';
...
describe('TransactionContext — addTransaction', () => {
  it('creates a transaction with valid inputs', async () => {
    ...
    mockDb.getFirstAsync
      .mockResolvedValueOnce({ id: 'acc-1', currency: 'USD', type: 'checking', balance: 5000 }) // account
    ...
    await act(async () => {
      await result.current.addTransaction(
        'acc-1', 'cat-1', 50, 'Lunch', Date.now(), 'expense'
      );
    });
  ...
  it('rejects when account does not exist', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addTransaction(
          'nonexistent', 'cat-1', 50, 'Test', Date.now(), 'expense'
        );
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects when category does not exist', async () => {
    mockDb.getFirstAsync
      .mockResolvedValueOnce({ id: 'acc-1', currency: 'USD', type: 'checking', balance: 5000 }) // account exists
    ...
    await expect(
      act(async () => {
        await result.current.addTransaction(
          'acc-1', 'nonexistent', 50, 'Test', Date.now(), 'expense'
        );
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects zero amount', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addTransaction(
          'acc-1', 'cat-1', 0, 'Test', Date.now(), 'expense'
        );
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects negative amount', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addTransaction(
          'acc-1', 'cat-1', -50, 'Test', Date.now(), 'expense'
        );
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects invalid transaction type', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addTransaction(
          'acc-1', 'cat-1', 50, 'Test', Date.now(), 'transfer' as any
        );
      })
    ).rejects.toThrow();
  });
...
describe('TransactionContext — Query Helpers', () => {
  beforeEach(() => {
    ...
    mockTransactions.push(
      {
        id: 'txn-1', accountId: 'acc-1', categoryId: 'cat-1', amount: 100,
        currency: 'USD', description: 'Income', date: new Date(thisYear, thisMonth, 5).getTime(),
        type: 'income', createdAt: 1,
      },
      {
        id: 'txn-2', accountId: 'acc-1', categoryId: 'cat-2', amount: 50,
        currency: 'USD', description: 'Expense', date: new Date(thisYear, thisMonth, 10).getTime(),
        type: 'expense', createdAt: 2,
      },
      {
        id: 'txn-3', accountId: 'acc-2', categoryId: 'cat-1', amount: 200,
        currency: 'EUR', description: 'Old', date: new Date(thisYear, thisMonth - 2, 15).getTime(),
        type: 'expense', createdAt: 3,
      },
    );
  });
