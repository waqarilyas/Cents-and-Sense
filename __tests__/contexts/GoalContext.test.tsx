...
describe('GoalContext — addGoal', () => {
  ...
  it('rejects negative target amount', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addGoal('Goal', -100, Date.now() + 86400000, 'USD');
      })
    ).rejects.toThrow();
  });
  ...
  it('rejects target amount exceeding max', async () => {
    ...
    await expect(
      act(async () => {
        await result.current.addGoal('Goal', 10000000000, Date.now() + 86400000, 'USD');
      })
    ).rejects.toThrow();
  });
});
...
describe('GoalContext — updateGoal', () => {
  ...
  it('updates goal name', async () => {
    ...
    await act(async () => {
      await result.current.updateGoal('goal-1', 'New Name', 5000, 1000, Date.now() + 86400000 * 90);
    });
  ...
  it('updates target amount', async () => {
    ...
    await act(async () => {
      await result.current.updateGoal('goal-1', 'Old Name', 10000, 1000, Date.now() + 86400000 * 90);
    });
...
describe('GoalContext — Query Helpers', () => {
  beforeEach(() => {
    ...
    mockGoals.push(
      { id: 'g1', name: 'Active', targetAmount: 1000, currentAmount: 500, currency: 'USD', deadline: future, createdAt: 1 },
      { id: 'g2', name: 'Completed', targetAmount: 500, currentAmount: 500, currency: 'USD', deadline: future, createdAt: 2 },
      { id: 'g3', name: 'Overdue', targetAmount: 1000, currentAmount: 200, currency: 'USD', deadline: past, createdAt: 3 },
    );
  });
  ...
  it('getActiveGoals returns goals where currentAmount < targetAmount', async () => {
    ...
    const active = result.current.getAllGoals().filter(g => g.currentAmount < g.targetAmount);
  ...
  it('getCompletedGoals returns goals where currentAmount >= targetAmount', async () => {
    ...
    const completed = result.current.getAllGoals().filter(g => g.currentAmount >= g.targetAmount);
